...
| `CAMERA_FRAME` | 카메라 프레임 (실시간) | **바이너리 전송**(권장): `[4B JSON 길이 LE][JSON][이미지 바이트]`. `format: "raw"` 시 백엔드가 **RGBA** (width×height×4)로 전송 → 프론트는 변환 없이 캔버스에 직접 사용. 프론트는 `CameraFrameData` (`imageBlob`/`imageBuffer`). 텍스트 폴백: base64 → 서비스에서 Blob/ArrayBuffer로 변환. |
