...
/** 뷰가 전체 이미지 경계에 닿았는지 판단할 때 쓰는 허용 오차 */
const VIEW_EDGE_EPS = 1e-2;
...
const Game02Play: React.FC<Game02PlayProps> = ({
  ...
  const atLeft = viewTopLeft.x <= VIEW_EDGE_EPS;
  const atRight = viewTopLeft.x >= 1 - viewWindow.w - VIEW_EDGE_EPS;
  const atTop = viewTopLeft.y <= VIEW_EDGE_EPS;
  const atBottom = viewTopLeft.y >= 1 - viewWindow.h - VIEW_EDGE_EPS;
      ...
      <aside className="w-[520px] shrink-0 h-full p-6 flex flex-col gap-6 game02-sidebar border-r border-white/10">
        ...
        <div className="flex gap-4">
          <div className="flex-1 rounded-[2rem] bg-zinc-900/60 border border-white/10 p-5">
            ...
            <div
              className={`text-7xl font-black font-mono leading-none ${timeLeft < 30 ? 'text-rose-500 animate-game02-timer-warning' : 'text-white'
                }`}
            >
              {formatTime(timeLeft)}
            </div>
        ...
        {SETTINGS.DEBUG_MODE && (
          <div className="rounded-[2rem] bg-zinc-900/60 border border-white/10 p-5">
            ...
            <div className="flex items-center gap-2 mb-2">
              <div
                className={`w-3 h-3 rounded-full ${viewPoseStatus.connected ? 'bg-green-500 animate-pulse' : 'bg-red-500'
                  }`}
              />
          ...
          >
            ...
            {atLeft && (
              <div
                ...
                style={{
                  background: 'linear-gradient(to right, rgba(255, 10, 10, 0.95), rgba(255, 10, 10, 0.4), transparent)',
                }}
                aria-hidden
              />
            )}
            {atRight && (
              <div
                ...
                style={{
                  background: 'linear-gradient(to left, rgba(255, 10, 10, 0.95), rgba(255, 10, 10, 0.4), transparent)',
                }}
                aria-hidden
              />
            )}
            {atTop && (
              <div
                ...
                style={{
                  background: 'linear-gradient(to bottom, rgba(255, 10, 10, 0.95), rgba(255, 10, 10, 0.4), transparent)',
                }}
                aria-hidden
              />
            )}
            {atBottom && (
              <div
                ...
                style={{
                  background: 'linear-gradient(to top, rgba(255, 10, 10, 0.95), rgba(255, 10, 10, 0.4), transparent)',
                }}
                aria-hidden
              />
            )}
