...
const SelectMinigame: React.FC<SelectMinigameProps> = ({ onComplete }) => {
  const baseGames = [
    ...
    { id: 'GAME03', name: 'ROBOT PUZZLE', icon: '🧩', color: 'from-green-500 to-teal-600' },
    { id: 'GAME04', name: 'MEMORY MATCH', icon: '🧠', color: 'from-pink-500 to-rose-600' },
    { id: 'GAME05', name: 'LASER DODGE', icon: '🛰️', color: 'from-cyan-500 to-blue-600' },
  ];
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center bg-slate-950 overflow-hidden relative" style={{ perspective: '3000px' }}>
      ...
      <div className="absolute inset-0 flex items-center justify-center z-20 pointer-events-none">
        <div className={`w-[440px] h-[440px] border-[10px] rounded-[3.5rem] transition-all duration-1000 flex flex-col items-center justify-center ${
          isFinalized ? 'border-white shadow-[0_0_150px_rgba(255,255,255,0.4)]' : 'border-blue-500 shadow-[0_0_60px_rgba(59,130,246,0.5)]'
        }`}>
          <div className={`absolute -top-8 px-12 py-2 text-2xl font-black rounded-full uppercase tracking-[0.3em] text-white shadow-2xl transition-all duration-500 ${
            isFinalized ? 'bg-white text-black scale-110' : 'bg-blue-600'
          }`}>
            TARGET
          </div>
        </div>
      ...
      >
        {displayGames.map((game, idx) => {
          ...
          return (
            <div
              key={idx}
              className={`relative flex-shrink-0 flex flex-col items-center justify-center rounded-[2.5rem] p-1 transition-all duration-700 ${
                isSelected 
